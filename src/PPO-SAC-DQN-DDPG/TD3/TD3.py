...
class Env:
    def __init__(self, action_dim):
        ...
        # 这里应该初始化环境，比如连接ROS主题或服务
        print("Environment initialized with action dimension:", action_dim)
        
    def reset(self):
        # 重置环境并返回初始状态
        print("Environment reset")
        # 返回模拟的初始状态 (25维)
        return np.random.rand(25) * 2 - 1
    
    def step(self, action, past_action):
        # 执行动作并返回新状态、奖励和是否结束
        # 这里是模拟实现，实际应与ROS环境交互
        new_state = np.random.rand(25) * 2 - 1
        reward = np.random.rand() * 2 - 1  # 随机奖励
        done = np.random.rand() > 0.95  # 5%的概率结束
        
        # 打印环境交互信息
        print(f"Env step: action={action}, reward={reward:.3f}, done={done}")
...
if __name__ == '__main__':
    ...
    # 训练循环
    # agent.load_models(episode=agent.start_epoch)  # 取消注释以加载已训练模型
    for e in range(agent.start_epoch + 1, 50000):
        ...
        for t in range(episode_step):
            ...
            print(f"ep {e}, step {t}")
            print(f"done: {done}, reward: {reward:.3f}, action: [{action[0]:.3f}, {action[1]:.3f}]")
            print(f"state (first 5 dims): {state[:5]}")
            print(f"reward_sum: {episode_reward_sum:.3f}, v = {action[0]:.3f}, w = {action[1]:.3f}\n")
        ...
        if e % 5 == 0:
            ...
            with open(SAVE_FIGURE_PATH + 'score_history.txt', 'w') as f:
                for score in score_history:
                    f.write(str(score) + '\n')
            print(f"Episode {e} completed, average reward over last 100: {np.mean(score_history[-100:]):.3f}")